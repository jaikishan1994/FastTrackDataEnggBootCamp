...
USE practice_db;
/*  CREATE CITY TABLE */
CREATE TABLE IF NOT EXISTS CITY (ID varchar(10), NAME varchar(100), COUNTRYCODE varchar(30), DISTRICT varchar(90), POPULATION varchar(8));
...
/* ALTER a couple of fields to numeric */
ALTER TABLE CITY MODIFY ID int;
ALTER TABLE CITY MODIFY POPULATION bigint;

CREATE TABLE IF NOT EXISTS STATION (ID INT, CITY VARCHAR(100), STATE VARCHAR(2), LAT_N INT, LONG_W INT);
...
CREATE TABLE IF NOT EXISTS PRODUCT(product_id int PRIMARY KEY, product_name varchar(100), unit_price int);
...
CREATE TABLE IF NOT EXISTS Ads(ad_id int, user_id int, action varchar(20));
...
CREATE TABLE IF NOT EXISTS Countries(country_id int, country_name varchar(25), PRIMARY KEY(country_id));
CREATE TABLE IF NOT EXISTS Weather(country_id int, weather_state int, day date, FOREIGN KEY(country_id) REFERENCES Countries(country_id));
...
CREATE TABLE IF NOT EXISTS UnitsSold(product_id int, purchase_date date, units int, FOREIGN KEY(product_id) REFERENCES Prices(product_id));
